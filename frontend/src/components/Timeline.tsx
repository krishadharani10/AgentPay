      ...
      {items.map((item, index) => {
        ...
        return (
          <div key={index} className="relative group">
            {/* Timeline node bead */}
            <div
              className={`absolute -left-6 top-0.5 w-5 h-5 rounded-full border ${theme.border} ${theme.bg} ${theme.text} flex items-center justify-center shadow-xs`}
            >
              {item.icon}
            </div>

            {/* Event content */}
            <div className="bg-white border border-slate-200 rounded-lg p-3.5 hover:border-slate-300 transition-colors shadow-xs">
                  ...
                  <span
                    className={`text-[10px] font-medium px-2 py-0.5 rounded ${
                      item.status === 'SUCCESS' || item.status === 'APPROVED' || item.status === 'SELECTED'
                        ...
                        : 'bg-blue-50 text-[#305EFF] border border-blue-200'
                    }`}
                  >
                    {item.status}
                  </span>
